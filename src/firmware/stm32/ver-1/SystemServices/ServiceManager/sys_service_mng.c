...
BaseStatusTypeDef SYS_SERVICE_MNG_Init(void)
{
  ...
  ret = SYS_DATA_MNG_SubscribeTopic(SYS_DATA_MNG_TOPIC_ACQ_SYS_TO_USB, SYS_Serial_SendRawSamples);
  __ASSERT(ret == BS_OK, BS_ERROR);

  ret = SYS_DATA_MNG_CreateTopic(SYS_DATA_MNG_TOPIC_DSP_TO_USB, SYS_ACQ_MNG_NUM_OF_EEG_CHANNEL*4, 10);
  __ASSERT(ret == BS_OK, BS_ERROR);

  ret = SYS_DATA_MNG_SubscribeTopic(SYS_DATA_MNG_TOPIC_DSP_TO_USB, SYS_Serial_SendFilteredSamples);
  __ASSERT(ret == BS_OK, BS_ERROR);
