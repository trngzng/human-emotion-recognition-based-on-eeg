...
/* USER CODE BEGIN PV */
//SYS_SERIAL_PacketTypeDef packetBuf = {0xAA,
//                                      0,
//                                      0,
//                                      {0},
//                                      0xF0F0,
...
int main(void)
{
  ...
  MX_FMAC_Init();
  /* USER CODE BEGIN 2 */
//  uint32_t *pTemp = (uint32_t *)&testVar;
//  packetBuf.data[0] = (uint8_t)(*pTemp >> 24);
//  packetBuf.data[1] = (uint8_t)(*pTemp >> 16);
//  packetBuf.data[2] = (uint8_t)(*pTemp >> 8);
//  packetBuf.data[3] = (uint8_t)(*pTemp >> 0);
//  packetBuf.length = 4;

  SYS_Serial_Init(&vcp, &hUsbDeviceFS);
